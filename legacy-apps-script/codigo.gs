...
function doPost(e) {
  ...
  if (!lock.tryLock(10000)) {
    return ContentService.createTextOutput(JSON.stringify({ "result": "error", "message": "El sistema está ocupado. Intente nuevamente." })).setMimeType(ContentService.MimeType.JSON);
  }
  ...
  try {
    ...
    if (!datos || !datos.items || !Array.isArray(datos.items) || !datos.cliente) {
      throw new Error("Payload inválido: faltan campos requeridos (items o cliente).");
    }
...
function procesarPedido(datos) {
  const lock = LockService.getScriptLock();
  if (!lock.tryLock(10000)) {
    throw new Error("El sistema está ocupado. Intente nuevamente.");
  }
...
function ajustarItemMasivo(ajustes) {
  const lock = LockService.getScriptLock();
  if (!lock.tryLock(10000)) {
    return "Error: El sistema está ocupado. Intente nuevamente.";
  }
  try {
    const sheetDetalle = SS.getSheetByName("DETALLE_PEDIDOS");
    const sheetPedidos = SS.getSheetByName("PEDIDOS");
      ...
      for (let i = 1; i < dataDetalle.length; i++) {
        if (String(dataDetalle[i][0]) === String(ajuste.idPedido) && String(dataDetalle[i][1]) === String(ajuste.idProd)) {
          ...
          if (cantNueva > 0) {
              ...
              dataDetalle.splice(i, 1); // Mantener array en memoria sincronizado con la hoja
          }
...
function getPedidosCompletos() {
  try {
    if (!SS) throw new Error("No se pudo conectar con la hoja de cálculo principal. Verifique la vinculación del script.");
    
    const sheetPedidos = SS.getSheetByName("PEDIDOS");
    const sheetDetalle = SS.getSheetByName("DETALLE_PEDIDOS");
    
    if (!sheetPedidos) throw new Error("No se encontró la hoja 'PEDIDOS'.");
    if (!sheetDetalle) throw new Error("No se encontró la hoja 'DETALLE_PEDIDOS'.");

    const fullDataP = sheetPedidos.getDataRange().getValues();
    const fullDataD = sheetDetalle.getDataRange().getValues();
    
    // Si solo hay cabeceras o está vacío
    if (fullDataP.length <= 1) return [];

    const dataP = fullDataP.slice(1);
    const dataD = fullDataD.slice(1);
    
    // Procesar de más nuevo a más viejo
    return [...dataP].reverse().map(row => {
      ...
      const items = dataD.filter(d => String(d[0]) === id).map(d => {
        const cant = parseFloat(d[4]) || 0;
        const subt = parseFloat(d[5]) || 0;
        return {
          id_prod: d[1],
          nombre: d[2],
          detalle: d[3],
          cantidad: cant,
          subtotal: subt,
          precio: cant > 0 ? (subt / cant) : 0
        };
      });
      
      let fechaStr = row[1];
      if (row[1] instanceof Date) {
        fechaStr = row[1].toLocaleString();
      } else if (fechaStr) {
    ...
    logDebug("ERROR en getPedidosCompletos: " + err.toString());
    throw new Error("Fallo al obtener pedidos completa: " + err.message);
  }
}
