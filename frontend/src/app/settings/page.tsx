...
interface SystemNotification {
    ...
    audiencia: 'preventista' | 'tienda' | 'todos';
}
                ...
                <AnimatePresence mode="popLayout">
                    {notifications.length === 0 ? (
                        ...
                        >
                            ...
                            <p className="text-sm text-slate-500 font-medium">No hay avisos activos. Creá uno y elegí si es para Preventistas, Tienda o Todos.</p>
                        </motion.div>
                    ) : (
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 ml-1">
                                            <Bell size={10} /> Destinatarios
                                        </label>
                                        <div className="flex gap-2">
                                            {(['preventista', 'tienda', 'todos'] as const).map(op => (
                                                <button
                                                    key={op}
                                                    onClick={() => updateNotification(notif.id, { audiencia: op })}
                                                    className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all ${(notif.audiencia || 'todos') === op
                                                            ? op === 'preventista' ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/20'
                                                                : op === 'tienda' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20'
                                                                    : 'bg-slate-700 text-white'
                                                            : 'bg-slate-100 dark:bg-slate-800 text-slate-500'
                                                        }`}
                                                >
                                                    {op === 'preventista' ? '🧑‍💼 Preventistas' : op === 'tienda' ? '🛒 Tienda' : '🌐 Todos'}
                                                </button>
                                            ))}
                                        </div>
                                    </div>
